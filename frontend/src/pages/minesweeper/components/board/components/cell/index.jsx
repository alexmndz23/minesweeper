...
const Cell = memo(
  ({
    ...
    const buttonClassname = classNames("w-9 h-9 grid place-items-center", {
      "border-4 bg-neutral-500 border-t-neutral-400 border-l-neutral-400 border-b-neutral-600 border-r-neutral-600":
        !isRevealed,
      "border-2 bg-neutral-600 border-neutral-700": isRevealed,
      "!bg-red-500": isFirstMineRevealed,
    });

    const numberClassname = classNames("font-bold text-xl", {
      "text-blue-400": aroundMines === 1,
      "text-green-400": aroundMines === 2,
      "text-red-400": aroundMines === 3,
      "text-indigo-400": aroundMines === 4,
      "text-purple-400": aroundMines === 5,
      "text-teal-500": aroundMines === 6,
      "text-neutral-900": aroundMines === 7,
      "text-neutral-400": aroundMines === 8,
    });
      ...
      <button
        ...
        {isRevealed &&
          (isMine ? (
            ...
            aroundMines > 0 && (
              <span className={numberClassname}>{aroundMines}</span>
            )
          ))}
      </button>
    );
  ...
  (prev, next) =>
    ...
    prev.isFirstMineRevealed === next.isFirstMineRevealed,
);
